...
import { login } from "../redux/Ecommerce_Admin_trunk";
import { Admin_info } from "../redux/Ecommerce_Admin_slice";


const Forgot_password_OTP = ({ length = 6 }) => {
  ...
  const handleResendOTP = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        server_url + "/api/user/verify_resend_otp",
        {
          email,
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // API Call
      const response = await axios.post(
        server_url + "/api/user/verify_forgot_password",
        {
          otp,
          email,
        }
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center mb-4">
        ...
        <p
          className=" font-semibold text-sm m-1 cursor-pointer text-orange-700"
