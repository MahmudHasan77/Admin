    ...
    const fetchingSlider = async () => {
      try {
        ...
        const response = await axios.get(
          server_url + "/api/slider/get_home_slider"
        );
  ...
  const delete_slider = async (id) => {
    try {
      ...
      const response = await axios.delete(
        server_url + `/api/slider/delete_slider/${id}`,
            ...
            return (
              <div key={slider?._id}>
                ...
                >
                  ...
                  <input type="color" value={slider?.TextColor} disabled />
                  <h1 className=" text-sm">{slider?.TextAlign}</h1>
