...
const UploadSecondSlider = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!sliderData.category) {
      toast.error("Slider Category is required");
      return;
    }
    if (!sliderData.image) {
    ...
    try {
      ...
      formData.append("category", sliderData.category);
      formData.append("image", sliderData.image);

      const response = await axios.post(
        server_url + "/api/slider/add_second_slider",
        formData,
        {
          headers: {
  ...
  return (
    <Container>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className=" grid min-w-60 max-w-70 mx-5 my-5">
          <select
            value={sliderData.category}
            onChange={(e) =>
              setSliderData((prev) => ({ ...prev, category: e.target.value }))
            }
            className=" border border-gray-400 bg-white max-h-40 resize-none outline-none text-sm px-2 py-1.5 rounded-sm customshadow1"
          >
          ...
          {imageUrl && (
            <div className=" max-w-40 max-h-30 border border-dashed rounded-sm mx-5 my-5 relative">
              ...
              <RxCrossCircled
                onClick={() => {
                  setImageUrl(null),
                    setSliderData({ ...sliderData, image: null });
                }}
...
export default UploadSecondSlider;
